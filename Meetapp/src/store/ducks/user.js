...
export const reducer = createReducer(INITIAL_STATE, {
  ...
  [Types.USER_REGISTER_FAILURE]: state =>
    state.merge({ error: true, loading: false }),
  [Types.USER_EDIT_PROFILE_REQUEST]: state => state.merge({ loading: true }),
  [Types.USER_EDIT_PROFILE_SUCCESS]: (state, { user }) =>
    state.merge({ user, loading: false, error: false }),
  [Types.USER_EDIT_PROFILE_FAILURE]: state =>
    state.merge({ error: true, loading: false }),
});
